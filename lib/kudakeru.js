...
function kudakeru (kudakeruCallback) {
  ...
  let upsertCookie = function (kid) {
    ...
    Cookies.set(cookieName, kid + '.' + timestamp(), {
      expires: 365,
  ...
  let sendData = function (queryData) {
    ...
    jsonp(getTrackingUrl(), queryData).then(function (responseData) {
      ...
      if (!responseData.kid) {
        ...
        dataLayerPush(cookieName + 'Error', { 'error': responseData })
        return false
      }
      ...
      dataLayerPush(cookieName + 'Loaded', responseData)
      if (responseData.fbel && window.fbq) {
        window.fbq('trackCustom', responseData.fbel)
      }
    }).catch(function (ex) {
      dataLayerPush('Error', { 'error': ex })
    })
  }
  setTimeout(function () {
    let queryData = {
    ...
    let oldCookie = Cookies.get(cookieName)
